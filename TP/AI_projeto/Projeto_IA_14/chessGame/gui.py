...
from model_integration import get_detected_board
from ai_move_explainer import explain_move_with_groq

def render_board(board, board_size):
    """
    Renderiza o tabuleiro de xadrez como uma imagem Pygame a partir de SVG.

    Args:
        board (chess.Board): O estado atual do tabuleiro.
        board_size (int): Tamanho do tabuleiro em pixeis.

    Returns:
        pygame.Surface: Imagem do tabuleiro renderizada.
    """
...
def get_square_from_mouse(pos, board_size):
    """
    Converte coordenadas do rato para uma casa do tabuleiro de xadrez.

    Args:
        pos (tuple): Posição do rato (x, y).
        board_size (int): Tamanho total do tabuleiro.

    Returns:
        int: Índice da casa (0 a 63).
    """
...
def capture_board_from_screen(surface, board_size, save_path="board.png"):
    """
    Captura uma imagem do tabuleiro renderizado e guarda como ficheiro.

    Args:
        surface (pygame.Surface): Superfície completa da aplicação.
        board_size (int): Tamanho do tabuleiro.
        save_path (str, optional): Caminho para guardar a imagem. Default é 'board.png'.
    """
...
def launch_main_gui(existing_board=None):
    """
    Inicia a interface gráfica principal da aplicação AI Chess Assistant.

    Permite interação com o tabuleiro, deteção de estado via imagem,
    sugestões da engine e explicações com linguagem natural.

    Args:
        existing_board (chess.Board, optional): Tabuleiro inicial a usar. Se inválido ou None, cria novo.
    """
    pygame.init()
    pygame.font.init()
